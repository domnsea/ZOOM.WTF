...
check_grep "macos launch opens Zoom with open -a" "open -a" \
  platforms/macos/1132.WTF.app/Contents/Resources/launch_fresh_zoom.sh
check_grep "macos launch still stages a runtime home folder" "runtime-home" \
  platforms/macos/1132.WTF.app/Contents/Resources/launch_fresh_zoom.sh
...
check_grep "macos launch sets the Mac full name" "1132wtf-setname.sh" \
  platforms/macos/1132.WTF.app/Contents/Resources/launch_fresh_zoom.sh
check_grep "macos launch opens Zoom as the logged-in desktop user" 'sudo -u "$CONSOLE_USER"' \
  platforms/macos/1132.WTF.app/Contents/Resources/launch_fresh_zoom.sh
check_grep "macos launch uses launchctl asuser" "launchctl asuser" \
  platforms/macos/1132.WTF.app/Contents/Resources/launch_fresh_zoom.sh
check_grep "macos launch uses open -a so a window appears" "open -a" \
  platforms/macos/1132.WTF.app/Contents/Resources/launch_fresh_zoom.sh
...
if "CFFIXED_USER_HOME" in helper and "open -a" not in helper:
    raise SystemExit("1.0.20 must open Zoom with open -a on this desktop")
if "open -a" not in helper:
    raise SystemExit("launch must open Zoom with open -a so a window appears")
if "CONSOLE_USER" not in helper or "sudo -u" not in helper or "launchctl asuser" not in helper:
    raise SystemExit("launch must run Zoom as the logged-in desktop user")
if "1132wtf-setname" not in helper:
    raise SystemExit("launch must set RealName so Join is not the Mac login")
if "sysadminctl -addUser" in helper:
    raise SystemExit("1.0.20 must not require a throwaway user to open Zoom")
if "throwaway user, so it was not opened" in helper:
    raise SystemExit("launch must not refuse to open Zoom as the login desktop")
if "pgrep -u" not in helper:
    raise SystemExit("launch must wait until Zoom is running on this desktop")
...
if "1.0.20" not in launcher or "1.0.20" not in gui:
    raise SystemExit("window must say 1.0.20 so the old app is obvious")
...
print("  ok    macos fix opens Zoom on this desktop with a random Join name")
PY
