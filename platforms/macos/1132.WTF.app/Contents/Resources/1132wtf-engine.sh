...
# Open Zoom on this desktop as the logged-in user after wiping identity
# and setting the Mac full name to a random Join name. 1.0.19 tried a
# throwaway OS user and refused to open Zoom when that GUI launch failed,
# which is why Mac never worked.
do_v9_launch() {
  local helper
  ...
  log ACTION "Opening factory-fresh Zoom on this desktop."
  log INFO "macOS will ask for your password once to reset Zoom and set the Join name."

  if ! /bin/bash "$helper" "$name"; then
    die "Fresh Zoom did not open on this desktop. See $HOME/Library/Logs/1132.WTF/launch.log"
  fi
  name="$(/usr/bin/tr -d '\r\n' <"$LOG_DIR/last_display_name.txt" 2>/dev/null || true)"
  [ -n "$name" ] || name="a new random name"
  if [ -z "$meeting" ]; then
    printf '%s' "$name" | /usr/bin/pbcopy >/dev/null 2>&1 || true
  fi
  log OK "Fresh Zoom is opening as '$name'."
  log DONE "Close Zoom completely, or run STEP 2, to restore your Mac full name and regular Zoom."
}
